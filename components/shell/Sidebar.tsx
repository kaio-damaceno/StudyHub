...
const Sidebar: React.FC<SidebarProps> = ({ 
  onHome, onTasks, onSchedule, onLibrary, onNotes, onFlashcards, onMindMap, onDocuments, onSettings, activeView 
}) => {
  ...
  return (
    <div className="w-[50px] bg-[#0f1223] flex flex-col items-center py-4 border-r border-blue-400/10 gap-4 z-[100] select-none">
      ...
      <div className="mt-auto flex flex-col gap-2 w-full px-1">
         ...
         <SidebarItem icon="settings" active={activeView === ViewState.SETTINGS} onClick={() => { setIsEditing(false); onSettings(); }} tooltip="Configurações" />
      </div>
    </div>
  );
};
