...
const DARK_MODE_INJECT = `
  (function() {
     ...
     // Aplica dark mode forçado ou ajustes visuais para principais motores
     if (hostname.includes('google.com') || hostname.includes('bing.com') || hostname.includes('duckduckgo.com') || hostname.includes('yahoo.com')) {
        ...
        style.textContent = \`
           ...
           /* DDG Specifics */
           .site-wrapper, .result { background-color: #202124 !important; }
...
// User Agent oficial do Chrome atualizado (v134) para garantir compatibilidade com CWS
  ...
  useEffect(() => {
    ...
    if (wv && tabId) {
      const handleDomReady = () => {
          try {
              wv.executeJavaScript(DARK_MODE_INJECT);
          } catch (e) { }
      };
      ...
      const handleInputEvent = (e: any) => {
          ...
          if (e.key === 'F5' || ((e.control || e.meta) && e.key === 'r')) { wv.reload(); return; }
      ...
      const handleNavigate = (e: any) => {
        if (e.httpResponseCode === 404) setErrorType('404');
        else if (e.httpResponseCode === 403) setErrorType('403');
        else if (e.httpResponseCode === 402) setErrorType('402');
        else if (e.httpResponseCode === 401) setErrorType('401');
        else setErrorType(null);

        if (e.url !== url && callbacksRef.current.onUpdateTab) {
           // Tenta obter o título atual da webview para evitar que o histórico salve apenas a URL
      ...
      const handleTitleUpdate = (e: any) => {
         if (callbacksRef.current.onUpdateTab) {
             ...
             // Também atualiza o histórico se o título mudar (importante para sites que carregam o título depois)
             if (callbacksRef.current.onAddToHistory) {
      ...
      const handleIpcMessage = (e: any) => {
         ...
         // ALT+CLICK DOWNLOAD TRIGGER
         if (e.channel === 'download-request') {
             const url = e.args[0];
             if (url) {
                 // Trigger native electron download
                 wv.downloadURL(url); 
             }
         }
      };
      ...
      const handleNewWindow = (e: any) => {
        ...
        // Regras de clique com modificadores
      ...
      const handleContextMenuEvent = (e: any) => {
          ...
          if (params.linkURL) {
              ...
              items.push({ 
                  label: 'Abrir em Segundo Plano', 
                  icon: 'layers', 
                  onClick: () => callbacksRef.current.onOpenNewTab && callbacksRef.current.onOpenNewTab(params.linkURL, true) 
              });
              ...
              items.push({ 
                  ...
                  icon: 'copy', 
                  onClick: () => {
                      if (params.linkURL) navigator.clipboard.writeText(params.linkURL);
                  } 
              });
              items.push({ type: 'separator' });
          }
          ...
          if (params.mediaType === 'image' && params.srcURL) {
              ...
              items.push({
                  ...
                  icon: 'copy',
                  onClick: () => navigator.clipboard.writeText(params.srcURL)
              });
              items.push({
                  label: 'Salvar Imagem Como...',
                  icon: 'download',
                  onClick: () => wv.downloadURL(params.srcURL)
              });
              items.push({ type: 'separator' });
          }
          ...
          items.push({
              label: 'Imprimir...',
              icon: 'printer', // Fallback to a generic icon if printer not exists or use externalLink
              onClick: () => wv.print()
          });

          // Mock do evento React.MouseEvent para o hook de ContextMenu
          // Precisamos das coordenadas absolutas da janela
          const rect = wv.getBoundingClientRect();
          const fakeEvent = {
              preventDefault: () => {},
              stopPropagation: () => {},
              clientX: rect.left + params.x,
              clientY: rect.top + params.y
      ...
      const handleFailLoad = (e: any) => {
          ...
          if (e.errorCode === -106 || e.errorCode === -105 || (e.errorDescription && (e.errorDescription.includes('INTERNET_DISCONNECTED') || e.errorDescription.includes('NAME_NOT_RESOLVED')))) {
      ...
      wv.addEventListener('plugin-crashed', handleCrashed);
      // Evento de contexto nativo do Electron Webview
      ...
      return () => {
          try {
              ...
              wv.removeEventListener('plugin-crashed', handleCrashed);
  ...
  return (
    <div className="w-full h-full relative group flex flex-col">
        <webview
            ...
            src={url}
            preload={preloadPath}
            className={`w-full h-full bg-[#0a0e27] ${errorType ? 'hidden' : ''}`}
        ...
        {errorType && (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#0a0e27]">
                <img 
                    src={`${errorType === 'offline' ? 'offline.png' : errorType === 'crash' ? 'crash.png' : `error-${errorType}.png`}`} 
                    alt={`Erro ${errorType}`} 
                    className="w-full h-full object-cover" 
                />
                {errorType === 'crash' && (
                    <div className="absolute bottom-10 text-white text-2xl font-bold bg-black/50 px-6 py-2 rounded-xl backdrop-blur-md">
        ...
        {showZoomControls && (
             <div className="absolute bottom-6 right-6 bg-black/70 backdrop-blur text-white px-4 py-2 rounded-full text-xs font-bold pointer-events-none transition-all animate-[fadeIn_0.2s_ease] border border-white/10 shadow-lg">
