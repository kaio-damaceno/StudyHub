...
const PDFFlashcardModal: React.FC<PDFFlashcardModalProps> = ({ initialFront, onClose }) => {
  ...
  useEffect(() => {
      ...
      else if (!targetDeckId && decks.length > 0) setTargetDeckId(decks[0].id);
  }, [decks, targetDeckId]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      if (finalDeckId) {
          addCard(finalDeckId, front, back, 'BASIC');
          onClose();
      }
  };

  return (
    <div className="fixed inset-0 z-[1200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease]">
      <div className="bg-[#14182d] border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
            <form id="pdf-flashcard-form" onSubmit={handleSubmit} className="space-y-4">
...
export default PDFFlashcardModal;
