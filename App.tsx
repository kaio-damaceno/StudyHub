...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
      // Notifica o Electron que o React terminou de renderizar a interface principal
      if (window.api && (window.api as any).appReady) {
          (window.api as any).appReady();
      }
  }, []);
  ...
  useEffect(() => {
      ...
      // Animações
      // Usamos uma classe no body para desabilitar animações se necessário, ou variáveis CSS
      if (!userSettings.animations?.enabled) {
          ...
          // Velocidade (Hack global para animações CSS puras seria complexo, mas podemos ajustar via JS onde possível)
          // Aqui mantemos simples: ligado/desligado para CSS.
      }
      ...
      <div className={`flex-1 flex flex-col min-w-0 relative ${userSettings.theme === 'ocean' ? 'bg-slate-900' : userSettings.theme === 'midnight' ? 'bg-black' : userSettings.theme === 'forest' ? 'bg-green-950' : userSettings.theme === 'sunset' ? 'bg-rose-950' : 'bg-[#0a0e27]'}`}>
        ...
        <div className={`flex-1 relative overflow-hidden flex flex-col mb-[65px] md:mb-0 ${userSettings.theme === 'ocean' ? 'bg-slate-900' : userSettings.theme === 'midnight' ? 'bg-black' : userSettings.theme === 'forest' ? 'bg-green-950' : userSettings.theme === 'sunset' ? 'bg-rose-950' : 'bg-[#0a0e27]'}`}>
          ...
          {activeTab?.viewState === ViewState.SEARCH && (
             <SearchView 
                query={activeTab.searchQuery || decodeURIComponent(activeTab.url.replace('studyhub://search/', ''))} 
                onNavigate={navigateActiveTab}
        ...
        <MobileNavBar 
            ...
            onToggleMenu={() => switchView(ViewState.SETTINGS)}
        />
      </div>
    </div>
  );
...
// App principal com os Providers
const App: React.FC = () => {
    return (
        <ContextMenuProvider>
        <OverlayProvider>
